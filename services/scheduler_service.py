...
from database.models import get_db, User, UserSubscription
...
class SchedulerService:
    ...
    async def add_subscription(self, user_id: int, subscription_type: str, chat_id: int) -> bool:
        ...
        try:
            ...
            if existing_subscription:
                ...
                logger.info(f"Обновлена подписка пользователя {user_id} на {subscription_type}")
            else:
                ...
                db.add(subscription)
                logger.info(f"Создана подписка пользователя {user_id} на {subscription_type}")
    ...
    async def remove_subscription(self, user_id: int, subscription_type: str) -> bool:
        ...
        try:
            ...
            if subscription:
                ...
                db.commit()
                logger.info(f"Отменена подписка пользователя {user_id} на {subscription_type}")
                return True
            else:
                return False
    ...
    async def _get_subscribers_by_type(self, subscription_type: str) -> List[Dict]:
        ...
        db = get_db()
        try:
            subscriptions = db.query(UserSubscription).join(User).filter(
                UserSubscription.subscription_type == subscription_type,
                UserSubscription.is_active == True,
                User.is_active == True
            ).all()
    ...
    async def cleanup_inactive_subscriptions(self) -> int:
        ...
        try:
            ...
            inactive_count = db.query(UserSubscription).join(User).filter(
    ...
    async def toggle_user_status(self, user_id: int, is_active: bool) -> bool:
        ...
        try:
            ...
            if user:
                ...
                db.commit()
                logger.info(f"Статус пользователя {user_id} изменен на {'активный' if is_active else 'неактивный'}")
                return True
            else:
                return False
