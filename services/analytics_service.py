...
class AnalyticsService:
        ...
        try:
            ...
            for group_data in self.monitored_groups:
                try:
                    ...
                    logger.error(f"Ошибка при сборе данных для группы {group_data['title']}: {e}")
    ...
    async def _collect_group_data(self, client: TelegramClient, group_data: Dict):
        ...
        try:
            ...
            if not hasattr(entity, 'participants_count'):
                logger.warning(f"Не удается получить количество участников для {group_data['title']}")
                return
            ...
            logger.warning(f"FloodWait для группы {group_data['title']}: {e.seconds} секунд")
    ...
    async def _collect_posts_data(self, client: TelegramClient, group_data: Dict, entity):
    ...
    async def _process_post(self, group_id: int, message):
        ...
        try:
            ...
            existing_post = db.query(GroupPost).filter(
                GroupPost.group_id == group_id,
                GroupPost.message_id == message.id
            ).first()
            ...
            if existing_post:
                ...
                if hasattr(message, 'reactions') and message.reactions:
                    await self._update_post_reactions(existing_post.id, message.reactions)
            else:
    ...
    async def _save_group_analytics(self, group_id: int, entity):
        ...
        try:
            ...
            existing_analytics = db.query(GroupAnalytics).filter(
                GroupAnalytics.group_id == group_id,
                GroupAnalytics.date >= datetime.combine(today, datetime.min.time()),
                GroupAnalytics.date < datetime.combine(today + timedelta(days=1), datetime.min.time())
            ).first()
            ...
            previous_analytics = db.query(GroupAnalytics).filter(
                GroupAnalytics.group_id == group_id,
                GroupAnalytics.date >= datetime.combine(yesterday, datetime.min.time()),
                GroupAnalytics.date < datetime.combine(yesterday + timedelta(days=1), datetime.min.time())
            ).first()
            ...
            if previous_analytics:
                ...
                if previous_analytics.members_count > 0:
                    members_growth_percent = (members_growth / previous_analytics.members_count) * 100
    ...
    async def _save_content_analytics(self, group_id: int, date):
        ...
        try:
            ...
            for content_type, stats in content_stats.items():
                avg_views = stats['total_views'] / stats['posts_count'] if stats['posts_count'] > 0 else 0
                avg_reactions = stats['total_reactions'] / stats['posts_count'] if stats['posts_count'] > 0 else 0
                
                # Проверка существующей записи
                existing = db.query(ContentAnalytics).filter(
