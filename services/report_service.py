...
from database.models import (
    get_db, TelegramGroup, GroupAnalytics, GroupPost, 
    PostReaction, ContentAnalytics
)
...
class ReportService:
    ...
    async def generate_daily_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            text_report = await self._generate_daily_text_report(groups_data, posts_data, yesterday)
    ...
    async def generate_weekly_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            groups_data = await self._get_groups_analytics_for_period(start_date, end_date)
            posts_data = await self._get_posts_analytics_for_period(start_date, end_date)
            content_data = await self._get_content_analytics_for_period(start_date, end_date)
            ...
            # Генерация графика
            chart_path = await self._generate_weekly_chart(groups_data, start_date, end_date)
    ...
    async def generate_monthly_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            text_report = await self._generate_monthly_text_report(
                groups_data, posts_data, content_data, best_worst_posts, start_date, end_date
            )
            
            # Генерация графика
            chart_path = await self._generate_monthly_chart(groups_data, start_date, end_date)
    ...
    async def generate_summary_report(self, target_date: date) -> Dict[str, Any]:
        ...
        try:
            ...
            groups_data = await self._get_groups_analytics_for_period(target_date, next_date)
            posts_data = await self._get_posts_analytics_for_period(target_date, next_date)
            
            # Генерация текстового отчета
            text_report = await self._generate_summary_text_report(groups_data, posts_data, target_date)
    ...
    async def _get_groups_analytics_for_period(self, start_date: date, end_date: date) -> List[Dict]:
        ...
        try:
            ...
            analytics = db.query(GroupAnalytics).join(TelegramGroup).filter(
    ...
    async def _get_posts_analytics_for_period(self, start_date: date, end_date: date) -> List[Dict]:
        ...
        try:
            ...
            posts = db.query(GroupPost).join(TelegramGroup).filter(
    ...
    async def _get_content_analytics_for_period(self, start_date: date, end_date: date) -> List[Dict]:
        ...
        try:
            ...
            content_analytics = db.query(ContentAnalytics).filter(
    ...
    async def _get_best_worst_posts(self, start_date: date, end_date: date) -> Dict:
        ...
        try:
            ...
            posts = db.query(GroupPost).join(TelegramGroup).filter(
                ...
                GroupPost.post_date < end_datetime,
                TelegramGroup.is_active == True
            ).order_by(GroupPost.views.desc()).all()
                ...
                'best': [
                    {
                        ...
                        'reactions_count': p.reactions_count,
                        'post_date': p.post_date,
                        'text_preview': (p.text[:50] + '...') if p.text and len(p.text) > 50 else p.text
                    }
                    for p in best_posts
                ],
                'worst': [
    ...
    async def _generate_daily_text_report(self, groups_data: List[Dict], posts_data: List[Dict], date: date) -> str:
        ...
        avg_views = sum(g['avg_views'] for g in groups_data) / len(groups_data) if groups_data else 0
        avg_reactions = sum(g['avg_reactions'] for g in groups_data) / len(groups_data) if groups_data else 0
...
📊 <b>Популярные типы контента:</b>"""
        ...
        recommendations = []
        if total_growth < 0:
            recommendations.append("Рост подписчиков замедлился. Рекомендуется проанализировать контент-стратегию")
        if avg_reactions < 10:
            recommendations.append("Низкая вовлеченность аудитории. Попробуйте более интерактивный контент")
        if total_posts == 0:
            recommendations.append("Активность публикаций низкая. Рекомендуется увеличить частоту постов")
    ...
    async def _generate_weekly_text_report(self, groups_data: List[Dict], posts_data: List[Dict], 
                                         content_data: List[Dict], start_date: date, end_date: date) -> str:
        ...
        for content in content_data:
            ...
            if content_type not in content_summary:
                content_summary[content_type] = {'posts': 0, 'avg_views': 0, 'avg_reactions': 0}
...
📊 <b>Анализ контента:</b>"""
        ...
        if content_summary:
            sorted_content = sorted(content_summary.items(), 
                                  key=lambda x: x[1]['posts'], reverse=True)
            for content_type, stats in sorted_content:
                ...
                avg_views = stats['avg_views'] / len([c for c in content_data if c['content_type'] == content_type])
        ...
        recommendations = []
        if total_growth < 0:
            recommendations.append("Отрицательная динамика подписчиков. Нужно пересмотреть контент-стратегию")
        if avg_daily_posts < 1:
            recommendations.append("Низкая активность публикаций. Рекомендуется публиковать минимум 1-2 поста в день")
    ...
    async def _generate_monthly_text_report(self, groups_data: List[Dict], posts_data: List[Dict],
                                          content_data: List[Dict], best_worst_posts: Dict,
        ...
        # Заменяем заголовок
        monthly_report = weekly_report.replace("Еженедельный отчет", "Ежемесячный отчет")
    ...
    async def _generate_summary_text_report(self, groups_data: List[Dict], posts_data: List[Dict], date: date) -> str:
        """Генерация текстового отчета за конкретную дату"""
        return await self._generate_daily_text_report(groups_data, posts_data, date)
    
    async def _generate_daily_chart(self, groups_data: List[Dict], date: date) -> Optional[str]:
        ...
        try:
            ...
            fig.suptitle(f'Ежедневная аналитика - {date.strftime("%d.%m.%Y")}', fontsize=16, fontweight='bold')
            ...
            # Сохранение графика
            chart_path = os.path.join(self.charts_dir, f'daily_report_{date.strftime("%Y%m%d")}.png')
    ...
    async def _generate_weekly_chart(self, groups_data: List[Dict], start_date: date, end_date: date) -> Optional[str]:
        ...
        try:
            ...
            period_str = f"{start_date.strftime('%d.%m')} - {end_date.strftime('%d.%m.%Y')}"
            fig.suptitle(f'Еженедельная аналитика - {period_str}', fontsize=16, fontweight='bold')
            
            # График 1: Динамика прироста подписчиков
            ax1.plot(daily_data['date'], daily_data['members_growth'], marker='o', linewidth=2, color='blue')
            ...
            ax2.bar(daily_data['date'], daily_data['posts_count'], color='orange', alpha=0.7)
            ax2.set_title('Активность публикаций')
            ax2.set_ylabel('Количество постов')
            ax2.grid(True, alpha=0.3)
            
            # График 3: Средние просмотры
            ax3.plot(daily_data['date'], daily_data['avg_views'], marker='s', linewidth=2, color='green')
            ...
            # Сохранение графика
            chart_path = os.path.join(self.charts_dir, f'weekly_report_{start_date.strftime("%Y%m%d")}_{end_date.strftime("%Y%m%d")}.png')
            plt.savefig(chart_path, dpi=300, bbox_inches='tight')
            plt.close()
    ...
    async def _generate_monthly_chart(self, groups_data: List[Dict], start_date: date, end_date: date) -> Optional[str]:
