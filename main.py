...
async def get_channel_analytics_data(start_date, end_date):
    ...
    try:
        ...
        story_views = 0
        story_likes = 0
        story_forwards = 0  # ✅ ИСПРАВЛЕНО: Добавлена отсутствующая переменная
        posts_by_hour = {}
        ...
        if posts_by_hour:
            ...
            # Топ-3 часа с корректным форматированием времени
...
async def summary_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if real_stats and isinstance(real_stats, dict) and 'title' in real_stats:
        ...
        await update.message.reply_text(
            ...
            f"⚡ Просмотры: Рассчитываются из истории\n"
            f"🎯 Охват: На основе реальных данных\n"
            f"🔄 Вовлеченность: См. /analiz для деталей\n\n"
            f"🔗 @{username}\n"
        ...
        await update.message.reply_text(
            ...
            "❌ <b>Канал не подключен</b>\n\n"
            "� <b>Необходимо настроить:</b>\n"
            "• CHANNEL_ID в переменных окружения\n"
            "• Подключение Telethon API\n"
            "• Права доступа к каналу\n\n"
            f"🆔 <b>Текущий CHANNEL_ID:</b> {CHANNEL_ID or 'не задан'}\n"
            f"📱 <b>Telethon статус:</b> {'✅ подключен' if telethon_client else '❌ не подключен'}\n\n"
            f"💡 <i>Используйте /help для инструкций по настройке</i>",
            parse_mode='HTML'
        )
...
async def growth_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if real_stats and isinstance(real_stats, dict):
        ...
        await update.message.reply_text(
            ...
            f"⚠️ <i>Telegram API не предоставляет историю подписчиков для каналов</i>\n"
            f"� <b>Рекомендация:</b> Используйте внешние сервисы аналитики\n\n"
        ...
        # Демо с честным предупреждением о ограничениях
        await update.message.reply_text(
            "📈 <b>Анализ роста канала</b>\n\n"
            
            "⚠️ <b>ОГРАНИЧЕНИЯ TELEGRAM API:</b>\n"
            "• Telegram не предоставляет историю подписчиков\n"
            "• Невозможно получить данные о приросте/оттоке\n"
            "• Доступно только текущее количество\n\n"
            
            "� <b>Рекомендации для отслеживания роста:</b>\n"
            "• Используйте TGStat, Telemetrio, ComBot\n"
            "• Ведите собственную статистику\n"
            "• Анализируйте вовлеченность постов\n\n"
            
            "🔧 <i>Демо-режим. Подключите внешние сервисы для точных данных</i>",
...
async def insights_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Получаем реальные данные для генерации инсайтов
    end_date = datetime.now(tz=pytz.timezone('Europe/Moscow'))
    ...
    if analytics_data and isinstance(analytics_data, dict):
        er = analytics_data.get('er', 0.0)
        posts_count = analytics_data.get('posts', 0)
        ...
        if posts_count > 0:
            avg_views = total_views // posts_count
            avg_reactions = total_reactions // posts_count
            ...
            best_hours_text = "\n".join([f"• {hour}: {er_val}" for hour, er_val in best_hours[:3]]) if best_hours else "• Данных недостаточно"
            
            insights_text = (
                f"🧠 <b>Маркетинговые инсайты: {channel_name}</b>\n\n"
                f"🌡️ <b>Качество вовлеченности:</b> {quality_level}\n"
                f"👥 <b>Аудитория:</b> {participants:,} подписчиков\n"
                f"📊 <b>ER (за 7 дней):</b> {er:.2f}%\n\n"
                f"📈 <b>Статистика контента (7 дней):</b>\n"
                f"• Постов: {posts_count}\n"
                f"• Средние просмотры: {avg_views:,}\n"
                f"• Средние реакции: {avg_reactions}\n\n"
                f"⏰ <b>Лучшие часы для публикаций:</b>\n"
                f"{best_hours_text}\n\n"
                f"� <b>Рекомендации:</b>\n"
                f"• Качество контента: {quality_text}\n"
                f"• Публикуйте в пиковые часы\n"
                f"• Следите за вовлеченностью\n\n"
                f"📊 <i>Данные основаны на реальной аналитике</i>"
            )
        else:
            insights_text = (
                f"🧠 <b>Маркетинговые инсайты: {channel_name}</b>\n\n"
                f"� <b>Аудитория:</b> {participants:,} подписчиков\n\n"
                f"⚠️ <b>Недостаточно данных для анализа</b>\n"
                f"• Нет постов за последние 7 дней\n"
                f"• Или нет доступа к истории канала\n\n"
                f"� <b>Рекомендации:</b>\n"
                f"• Опубликуйте контент для анализа\n"
                f"• Проверьте настройки Telethon\n"
        ...
        insights_text = (
            f"🧠 <b>Маркетинговые инсайты: {channel_name}</b>\n\n"
            f"� <b>Аудитория:</b> {participants:,} подписчиков\n\n"
            f"⚠️ <b>Нет доступа к аналитическим данным</b>\n"
            f"• Проверьте подключение Telethon\n"
            f"• Убедитесь в правильности CHANNEL_ID\n"
            f"• Проверьте права доступа к каналу\n\n"
            f"🔧 <i>Подключите Telethon для получения реальных инсайтов</i>"
        )
...
async def analiz_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Получаем реальные данные канала с аналитикой
        real_stats = await get_real_channel_stats()
        
        # Получаем расширенные аналитические данные
        end_date = datetime.now(tz=pytz.timezone('Europe/Moscow'))
        start_date = end_date - timedelta(days=7)
        analytics_data = await get_channel_analytics_data(start_date, end_date)
        
        # Объединяем данные для генератора изображений
        combined_stats = {}
        if real_stats:
            combined_stats.update(real_stats)
        if analytics_data:
            combined_stats.update(analytics_data)
        
        # Генерируем изображение с полными данными
        image_buffer = await generate_channel_analytics_image(combined_stats if combined_stats else None)
