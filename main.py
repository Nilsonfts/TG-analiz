...
async def growth_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if real_stats and isinstance(real_stats, dict):
        # Если есть реальные данные, показываем их
        ...
        await update.message.reply_text(
            ...
            "⚠️ <i>Историческая статистика появится после накопления данных</i>",
            parse_mode='HTML'
        )
    else:
        # Fallback с демо-данными
        await update.message.reply_text(
            "📈 <b>Рост подписчиков (7 дней)</b>\n\n"
            "📅 <b>Демо-статистика:</b>\n"
            "• Понедельник: +45 👥\n"
            ...
            "🏆 <b>Лучший день:</b> Пятница (+67)\n"
            "📍 <b>Средний прирост:</b> +44/день\n\n"
            "⚠️ <i>Подключите Telethon для реальных данных</i>",
            parse_mode='HTML'
        )
