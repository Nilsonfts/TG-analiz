...
async def daily_report_command(update, context):
    ...
    if analytics and analytics.get('access_confirmed') and real_stats:
        ...
        await update.message.reply_text(
            ...
            f"❤️ Всего реакций на посты: {total_post_reactions}\n"
            f"💝 Всего реакций на сторис: {total_story_reactions}\n"
            ...
            f"🔥 Температура канала: {analytics.get('temperature', '⬜⬜⬜⬜⬜')} {analytics.get('temperature_score', '(0/5)')}\n"
            f"   <i>(5 огоньков = горячий канал, высокая активность)</i>\n"
            f"📈 Рейтинг ER: {analytics.get('er_rating', 'Средне')}\n"
            f"   <i>(Оценка вовлеченности: Отлично/Хорошо/Средне/Плохо)</i>\n\n"
            
            f"⚠️ <b>Ограничения:</b>\n"
            f"• Точные подписки/отписки недоступны через публичный API (показаны оценки)\n"
            f"• Сторис определяются алгоритмом (видео ≤60сек + фото с минимумом текста)\n"
