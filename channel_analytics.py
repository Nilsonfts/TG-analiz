...
class ChannelAnalytics:
    ...
    async def create_channel_tables(self):
        ...
        async with self.pool.acquire() as conn:
            ...
            # Индексы
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_channel_posts_date ON channel_posts(channel_id, publish_date)"
            )
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_subscriber_analytics_date ON subscriber_analytics(channel_id, date)"
            )
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_views_analytics_date ON views_analytics(channel_id, date)"
            )
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_traffic_sources_date ON traffic_sources(channel_id, date)"
            )
    ...
    async def get_channel_summary(self, channel_id: int) -> dict[str, Any]:
        ...
        async with self.pool.acquire() as conn:
            ...
            growth_percentage = (
                (subscriber_growth / current_subscribers * 100)
            ...
            if notifications_data and notifications_data["total_subscribers"] > 0:
                notifications_enabled_percent = (
                    ...
                    / notifications_data["total_subscribers"]
                    * 100
                )
    ...
    async def get_subscriber_growth_data(
    ...
    async def get_hourly_views_data(
    ...
    async def get_traffic_sources_data(
        self, channel_id: int, days: int = 30
    ) -> list[dict[str, Any]]:
    ...
    async def generate_recommendations(self, channel_id: int) -> list[str]:
        ...
        try:
            ...
            if summary.get("notifications_enabled_percent", 0) < 50:
                ...
                recommendations.append(
                    "🔔 Мотивируйте подписчиков включить уведомления"
                )
            ...
            if views < subscribers * 0.3:  # Менее 30% охвата
                recommendations.append(
                    "📈 Низкий охват - оптимизируйте время публикации"
                )
                recommendations.append(
                    "⏰ Анализируйте пиковые часы активности аудитории"
                )

            # Анализ контента
            if summary.get("reactions_count", 0) < summary.get("posts_count", 1) * 0.1:
                recommendations.append(
                    "💬 Мало реакций - создавайте более интерактивный контент"
                )
            ...
            if not recommendations:
                ...
                recommendations.append(
                    "🚀 Экспериментируйте с новыми форматами контента"
                )
