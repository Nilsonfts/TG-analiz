...
async def init_channel_db():
    ...
    try:
        # Подключение к БД
        db = Database(f"postgresql://{DB_CONFIG['user']}:{DB_CONFIG['password']}@{DB_CONFIG['host']}:{DB_CONFIG['port']}/{DB_CONFIG['database']}")
        await db.init_db()
            ...
            if day >= 23:  # Последние 7 дней
                for hour in range(24):
                    ...
                    try:
                        ...
                        logger.debug(f"Почасовые данные за {date.date()} {hour}:00 уже существуют")
